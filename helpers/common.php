...
if ( ! function_exists('exec_request'))
{
	...
	function exec_request($config)
	{
		...
		$uri['method']     = isset($uri['param_arr'][0]) && in_array($uri['param_arr'][0], array_slice(array_keys($config['uri']['action_hash']), 1)) ? array_shift($uri['param_arr']) : array_keys($config['uri']['action_hash'])[0];
